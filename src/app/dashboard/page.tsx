    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-12">
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-green-500/5 border border-green-500/20 rounded-xl px-4 py-3 shadow-sm">
          ...
          <Link href="/dashboard/api-keys" className="text-xs font-semibold text-green-700 dark:text-green-400 hover:opacity-80 flex items-center gap-1.5 bg-green-500/10 px-3 py-1.5 rounded-lg transition-colors">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        {/* THE REFINED WALLET CARD - SCALED DOWN */}
        <div className="xl:col-span-2 bg-primary rounded-3xl p-6 lg:p-8 relative overflow-hidden shadow-lg shadow-primary/20 flex flex-col justify-between min-h-[200px] text-primary-foreground border border-primary/20">
          <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-gradient-to-br from-white/10 to-transparent rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none" />
          ...
          <div className="relative z-10 flex-1 flex flex-col justify-center mb-6">
            ...
            <div className="flex items-start gap-1">
              ...
              <span className="text-5xl lg:text-6xl leading-none font-bold tracking-tight">{balance}</span>
            </div>
          </div>

          <div className="relative z-10 pt-5 border-t border-primary-foreground/10 flex flex-wrap items-center gap-3">
            <button className="flex items-center gap-2 bg-white text-primary hover:bg-white/90 px-6 py-2.5 rounded-xl font-bold text-sm transition-all shadow-sm active:scale-95">
              <Plus className="h-4 w-4 stroke-[3]" /> Fund Wallet
            </button>
            <button className="flex items-center gap-2 bg-primary-foreground/10 text-primary-foreground hover:bg-primary-foreground/20 border border-primary-foreground/10 px-6 py-2.5 rounded-xl font-semibold text-sm transition-all active:scale-95 backdrop-blur-md">
        ...
        <div className="grid grid-cols-2 gap-4 h-full">
          <div className="bg-card border border-border/60 rounded-2xl p-4 flex flex-col justify-center shadow-sm">
            <Activity className="h-4 w-4 text-muted-foreground mb-3" />
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Today's Vends</p>
            <p className="text-xl font-bold text-foreground tracking-tight">1,284</p>
          </div>
          <div className="bg-card border border-border/60 rounded-2xl p-4 flex flex-col justify-center shadow-sm">
            ...
            <p className="text-xl font-bold text-green-500 tracking-tight">99.8%</p>
          </div>
          <div className="bg-card border border-border/60 rounded-2xl p-4 flex flex-col justify-center shadow-sm">
            <Key className="h-4 w-4 text-muted-foreground mb-3" />
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">API Calls (24h)</p>
            <p className="text-xl font-bold text-foreground tracking-tight">45.2K</p>
          </div>
          <div className="bg-card border border-border/60 rounded-2xl p-4 flex flex-col justify-center shadow-sm">
            <Webhook className="h-4 w-4 text-muted-foreground mb-3" />
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-1">Active Hooks</p>
            <p className="text-xl font-bold text-foreground tracking-tight">2 / 2</p>
          </div>
        </div>
      </div>
      ...
      <div>
        <h2 className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-4">Manual Operations</h2>
            ...
            <tbody className="divide-y divide-border/50">
              {RECENT_TRANSACTIONS.map((tx) => (
                <tr key={tx.id} className="hover:bg-muted/20 transition-colors">
                  <td className="px-6 py-3.5 font-mono text-muted-foreground text-xs">{tx.id}</td>
                  <td className="px-6 py-3.5 font-medium text-foreground text-[13px]">{tx.service}</td>
                  <td className="px-6 py-3.5 font-semibold text-foreground text-[13px]">
                  ...
                  <td className="px-6 py-3.5 text-right text-muted-foreground whitespace-nowrap text-xs">
